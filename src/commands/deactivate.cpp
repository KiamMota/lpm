...
void deactivate(std::vector<std::string> &args) {
  if (args.empty()) {
    cli::require_arg("deactivate", "<plugin name>");
    return;
  }
  ...
  for (std::string &l : lines) {
    if (l.find(require_line) != std::string::npos) {
      found = true;
      if (l.rfind("-- ", 0) == 0) {
        std::cout << "already deactivated." << std::endl;
        return;
      }
      l = "-- " + l;
      std::cout << "deactivated: " << require_line << std::endl;
      break;
    }
  }

  if (!found) {
    std::cout << "require not found in init.lua. use 'lpm include "
              << found_name << "' first." << std::endl;
    return;
  }
  ...
  for (const std::string &l : lines) {
    ...
    if (file_out.fail()) {
      file_out.close();
      rollback();
      std::cout << "err: failed to write, rolled back." << std::endl;
      return;
    }
  }
  file_out.close();
